...
function goto45() {		// clôture du tournoi
	var nextstring = "bridge45.php?idtournoi=" + idtournoi;
	location.replace( nextstring );
};
function goto45c() {		// correction d'une donne
	var nextstring = "howell45c.php?idtournoi=" + idtournoi;
	location.replace( nextstring );
};
function goto45m() {		// moyenne générale
	var nextstring = "howell45m.php?idtournoi=" + idtournoi;
	location.replace( nextstring );
};
function goto45s() {		// suppression d'une donne
	var nextstring = "howell45s.php?idtournoi=" + idtournoi;
	location.replace( nextstring );
};
function goto48() {		// Entrer les résultats manquants d'une donne
	var nextstring = "howell48.php?idtournoi=" + idtournoi;
	location.replace( nextstring );
};
function cdeplus() {
...
<body>
	...
	if ( ($t['idtype'] <= $min_type_affimp)&&($parametres['affimp']==1) ) {
		...
		$ok = displayTournoiIMP( $idtournoi );
	}
	else {
