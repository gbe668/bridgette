...
if ( isset($_POST['phrase3']) ) {
	...
	if($res->fetchColumn() == 1){
		...
		if ($newpassword1==$newpassword2) {
			...
			$res = $dbh->query($sql);
			if($res) {
				$message =  "Mot de passe changé avec succès.</br>Retournez sur l'application android";
				$okchange = 1;
			}
...
<body>
	...
	<script>
	let okchange= parseInt( "<?php echo $okchange; ?>" );
	if ( okchange > 0 )
		$('#masquebouton').addClass( 'section_invisible' );
	else
		$("#newpassword1").focus();
	</script></body>
</body>
